(function() {
      ...
      keyUpDown:function(event) {
        ...
        switch(event.keyCode) {
          ...
          case 40: controller.down = key_state; break; // down key
            // alternate movement options wasd
          case 65: controller.left = key_state; break; // left key alternate = a
          case 87: controller.up = key_state; break; // left key alternate = w
          case 68: controller.right = key_state; break; // left key alternate = d
          case 83: controller.down = key_state; break; // left key alternate = s
    ...
    display = {
      ...
      /* This object holds all graphics for the game. Each graphic is just
      ...
      graphics: {
        ...
        1: {// segment
          ...
          draw:function() {
            ...
            context.fillStyle = "#202830";
            context.fillRect(0, 0, this.canvas.width, this.canvas.height);
            context.fillStyle = "#81b3a7"; //snake color
        ...
        2: {// apple
          ...
          draw:function() {
            ...
            context.fillStyle = "#202830"; //apple outline
            context.fillRect(0, 0, this.canvas.width, this.canvas.height);
            context.fillStyle = "#d10031"; //apple color
            context.fillRect(1, 1, game.world.tile_size - 2, game.world.tile_size - 2);
      ...
      /* create variable names to simplify references */
      background_tile:0,
      segment:1,
      apple:2,
      ...
      render:function() {
        ...
        /* Loop through the board and draw the graphics. */
        for (let index = 0; index < game.world.board.length; index ++) {
  
          /* Get the appropriate graphics canvas from the graphics object. */
          let graphic = this.graphics[game.world.board[index]].canvas;
      ...
      resize:function(event) {
        ...
        /* Hide some elements on large screen sizes */
    ...
    game = {
      ...
      /* The apple records a location on the board. */
      apple: {
      ...
      /* snake object. tracks board indices
      and the movement. */
      snake: {
      ...
      /* world object holds information about the level. */
      world:{
  
        columns:20,
        tile_size:10,
        board:new Array(400).fill(display.background_tile)// Creates a new array with 400 spaces filled with 0s.
  
      },
  
      /* The amount of time accumulated */
      accumulated_time:0,
      ...
      reset:function() {
        ...
        for (let index = this.snake.segment_indices.length - 1; index > -1; -- index) {
          ...
          this.world.board[this.snake.segment_indices[index]] = display.background_tile;
        ...
        this.world.board[game.apple.index] = display.apple;
        this.world.board[game.snake.segment_indices[0]] = display.segment;
        this.world.board[game.snake.segment_indices[1]] = display.segment;
      ...
      loop:function(time_stamp) {
        ...
        if (time_stamp >= game.accumulated_time + game.time_step) {
          ...
          if (game.snake.vector_x != 0 || game.snake.vector_y != 0) {
            ...
            /* This allows the us to pause the game. If you move in the exact
            opposite direction it sets the movement vector on each axis to 0. */
            ...
            game.world.board[tail_index] = display.background_tile;// set the tail index in the board to 0
            ...
            if (game.world.board[game.snake.head_index] == display.segment// hit a segment
              || game.snake.head_index < 0// off the top of the board
              || game.snake.head_index > game.world.board.length - 1// off the bottom
              || (game.snake.vector_x == -1 && game.snake.head_index % game.world.columns == game.world.columns - 1)// off the left of the board
            ...
            game.world.board[game.snake.head_index] = display.segment;
            ...
            /* Did the snake touch the apple? */
            if (game.snake.head_index == game.apple.index) {
  
              game.score +=10; // each fruit eaten gives +10 points
              game.time_step = (game.time_step > 100)?game.time_step - 10:100;// increase speed
  
              // add to the tail
              game.snake.segment_indices.push(tail_index);
              game.world.board[tail_index] = display.segment;
              game.apple.index = Math.floor(Math.random() * game.world.board.length);// reset the apple
  
              // If the snake fills up the entire board minus 1 space, reset.
              if (game.snake.segment_indices.length == game.world.board.length - 1) {
              ...
              while(game.world.board[game.apple.index] != display.background_tile) {
  
                game.apple.index ++;
  
                // If checking past the bottom of the board, jump to the top
                if (game.apple.index > game.world.board.length - 1) {
              ...
              // Once loop ends, we have a spot for the apple.
              game.world.board[game.apple.index] = display.apple;
